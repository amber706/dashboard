-- ─────────────────────────────────────────────────────────────────
-- Migration 188 — DUI admits = Screening sold + Course sold (separate
-- events per deal, with their own dates).
--
-- Old logic counted each DUI deal once with stage_category = 'closed_won_
-- dui_completion', anchored on COALESCE(admit_date, closing_date). DUI
-- deals don't have admit_date, so all events landed on closing_date —
-- including screenings sold earlier on the deal lifecycle.
--
-- New logic (per Amber's CONFIRMED guidance):
--   • DUI screening sold  → 1 admit event on screening_closed_date
--   • DUI course sold     → 1 admit event on closing_date
--   • A deal with both    → 2 events on two different dates
--
-- Non-DUI pipelines (treatment + DV) unchanged:
--   admit_date IS NOT NULL OR stage_category = 'closed_won_admitted',
--   anchored on COALESCE(admit_date, closing_date).
--
-- Also widens op_conversion_rates_daily ratio columns from numeric(6,4)
-- to numeric(10,4) — DUI admit/vob can exceed 100% since DUI bypasses
-- VOB.
-- ─────────────────────────────────────────────────────────────────

ALTER TABLE reporting.op_conversion_rates_daily
  ALTER COLUMN lead_to_mql  TYPE numeric(10,4),
  ALTER COLUMN mql_to_vob   TYPE numeric(10,4),
  ALTER COLUMN vob_to_admit TYPE numeric(10,4),
  ALTER COLUMN mql_to_admit TYPE numeric(10,4);
...
BEGIN
  ...
  -- admits_count: treatment+DV (one event per deal) + DUI screenings + DUI courses
  WITH admit_events AS (
    SELECT COALESCE(d.admit_date, d.closing_date) AS event_date,
           d.owner_user_id, d.source_category, d.pipeline, d.admitted_level_of_care
    FROM reporting.deals d
    WHERE d.pipeline IS DISTINCT FROM 'dui_cash'
    ...
    UNION ALL
    SELECT d.screening_closed_date, d.owner_user_id, d.source_category, d.pipeline, d.admitted_level_of_care
    FROM reporting.deals d
    WHERE d.pipeline = 'dui_cash'
      AND d.screening_sold = TRUE
      AND d.screening_closed_date BETWEEN v_cutoff AND v_today
    UNION ALL
    SELECT d.closing_date, d.owner_user_id, d.source_category, d.pipeline, d.admitted_level_of_care
    FROM reporting.deals d
    WHERE d.pipeline = 'dui_cash'
  ...
  SELECT event_date, owner_user_id, source_category, pipeline, admitted_level_of_care, COUNT(*)
  FROM admit_events
  GROUP BY 1, 2, 3, 4, 5
  ON CONFLICT (date, owner_user_id, source_category, pipeline, level_of_care) WHERE TRUE
  ...
  -- Conversion rates: admit events match the build above (treatment+DV +
  -- DUI screenings + DUI courses) so MQL→Admit reflects the same total.
  ...
  admit_events_all AS (
    SELECT COALESCE(d.admit_date, d.closing_date) AS event_date, d.pipeline FROM reporting.deals d
    WHERE d.pipeline IS DISTINCT FROM 'dui_cash'
      AND (d.admit_date IS NOT NULL OR d.stage_category = 'closed_won_admitted')
    UNION ALL
    SELECT d.screening_closed_date, d.pipeline FROM reporting.deals d
    WHERE d.pipeline = 'dui_cash' AND d.screening_sold = TRUE AND d.screening_closed_date IS NOT NULL
    UNION ALL
    SELECT d.closing_date, d.pipeline FROM reporting.deals d
    WHERE d.pipeline = 'dui_cash' AND d.course_sold = TRUE AND d.closing_date IS NOT NULL
  ),
  with_counts AS (
    SELECT sr.date, sr.scope_dimensions,
      ...
      (SELECT COUNT(*) FROM admit_events_all a WHERE a.event_date BETWEEN sr.date - 29 AND sr.date AND (sr.pipeline_filter IS NULL OR a.pipeline = sr.pipeline_filter)) AS admits_n
    FROM scope_rows sr
  )
