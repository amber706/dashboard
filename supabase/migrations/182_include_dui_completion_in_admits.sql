-- ───────────────────────────────────────────────────────────────────────────
-- Migration 182 — Include DUI completions in admits aggregation
--
-- The original admits predicate excluded DUI deals because they never have
-- admit_date set and their stage_category is closed_won_dui_completion,
-- not closed_won_admitted. Result: dui_cash pipeline always showed 0
-- admits on /analytics/op-overview even though screening/classes-sold deals
-- exist (17 MTD as of 2026-05-29).
--
-- Fix: include closed_won_dui_completion in the admits predicate. Date
-- attribution falls back to closing_date via the existing COALESCE.
--
-- Top-line vs non-top-line accounting stays in the frontend (which already
-- excludes DUI/DV pipelines from the top-line KPI tile per CONFIRMED.md #3).
--
-- Predicate updated in three places:
--   1. op_lead_funnel_daily admits_count INSERT
--   2. op_conversion_rates_daily admits_n subquery
--   3. op_sales_cycle_daily (now also includes dui_cash in the pipeline set)
-- ───────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- admits_count — predicate now includes closed_won_dui_completion. DUI deals
  -- never have admit_date; date attribution falls back to closing_date.
  ...
  SELECT d.closing_date, d.owner_user_id, d.source_category, d.pipeline, d.level_of_care_requested, COUNT(*)
  FROM reporting.deals d
  WHERE d.stage_category = 'closed_won_referred_out_unattached' AND d.closing_date BETWEEN v_cutoff AND v_today
  GROUP BY 1, 2, 3, 4, 5
  ON CONFLICT (date, owner_user_id, source_category, pipeline, level_of_care) WHERE TRUE
